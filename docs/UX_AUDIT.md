...
- Factored Leap keeps its frozen full statement and, where the production font requires it, uses a third complete statement line. The graph viewport, clipping, scrollbar and hover origin move by the same one-line amount; one- and two-line theorems retain their prior two-line statement hitbox and graph origin.
